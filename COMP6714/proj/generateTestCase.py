...
if __name__ == '__main__':
    ...
    list.append(['UK'])
    list.append(['UK', 'UK'])
    list.append(['on', 'a', 'new', 'version'])
    list.append(['on', 'a', 'on', 'a', 'new', 'new', 'version', 'version'])
    list.append(['spark', 'a', 'new', 'Ink'])
    list.append(['spark', 'spark', 'spark', 'spark'])
    list.append(
        ['Digital', 'guru', 'floats', 'sub100', 'PC', 'Nicholas', 'Negroponte', 'chairman', 'and', 'founder', 'of',
         'MIT', 's', 'Media', 'Labs says'])
    for query in list:
        for top_k in range(1,7):
            print("query = ",query," top_k =",top_k)
            topk_result, full_evaluation_count = project_part1.WAND_Algo(query, top_k, inverted_index)
