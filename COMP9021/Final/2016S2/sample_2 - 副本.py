...
def f(n):
    ...
    n_factorial = n_factorial.strip('0')
    length = len(n_factorial)

    
    print(f'{n} factorial is {output}')
    if length !=1:
        print(f'It has {length} digits, the trailing 0\'s excepted')
    else:
        print(f'It has {length} digit, the trailing 0\'s excepted')
