...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    lexicon, close = get_words_and_word_relationships()
    result = False
    if word_1 and word_2 and candidate_ladder and word_1 in lexicon and word_2 in lexicon:
        if word_1 == candidate_ladder[0] and word_2 == candidate_ladder[-1]:
            if len(candidate_ladder) == 1:
                result = True
            else:
                ladders = candidate_ladder[::-1]
                word = ladders.pop()
                word_values = close[word]
                ...
                while ladders:
                    ...
                    if word == word_2:
                        result = True
                        break
                    if word not in word_values or word_2 in word_values:
                        break
                    word_values = close[word]
                else:
                    result = True
