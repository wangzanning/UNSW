...
create or replace view Q8_mark_fail_4(student,subject,mark)
as select distinct ce.student,co.subject,ce.mark from courses co 
	join course_enrolments ce on ce.course = co.id
	join Q8_all_subject_3 on Q8_all_subject_3.subject = co.subject
where ce.mark < 50 ; 

create or replace view Q8(zid, name)
as select distinct 'z'||M.student as zid,p1.name from Q8_mark_fail_4 M
