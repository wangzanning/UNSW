...
def c2lsh(data_hashes, query_hashes, alpha_m, beta_n):
    ...
    #change the data_hashes to [hashes,data, difference]
    data_hashes = data_hashes.map(lambda e: (e[0], e[1], check_diff(e[1], query_hashes)))

    offset = -1
    while True:
        offset = offset + 1
        #filter the data_hashes with satisfy the offset
        data_hashes = data_hashes.filter(lambda e: check_satisfy(e[2], alpha_m, offset))
